"""Dragapult ex — deck-specific rule-based agent.

BASELINE: this is the official Kaggle sample "Rule-Based Agent for Dragapult ex"
reproduced verbatim (Crispin aggressive variant), wired so it runs against our
local `cg` engine and our gate harness. We keep it byte-for-byte first so we can
**measure the baseline before improving it** (RULINGS Part 0, mindset #1/#5/#6).
Improvements over the sample go in clearly-marked blocks once the gate gives us a
number to beat.

Exposes `agent(obs_dict) -> list[int]`. Deck is resolved from (in order):
  1. env var DRAGAPULT_DECK
  2. agent_decks/dragapult_ex_sample.csv (repo default)
  3. /kaggle_simulations/agent/deck.csv (Kaggle runtime)
"""
import os
import sys
...
bench_attacker = False  # Whether there is a Benched Pokemon that is ready to attack
...
def _agent_impl(obs_dict: dict) -> list[int]:
    """Core scoring logic (the official sample). Wrapped by `agent` below for
    never-crash safety + output validation (IMPROVEMENT over the sample, Ruling R7)."""
    obs = to_observation_class(obs_dict)
    if obs.select is None:
        # In the initial selection, obs.select is None; return the 60-card deck.
        return my_deck
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card is not None:
                ...
                if (context == SelectContext.SWITCH
                    ...
                    # Selection of the Pokemon to send to the Active Spot
...
def _legal_fallback(obs_dict: dict) -> list[int]:
    ...
    if n == 0 or max_c == 0:
        return []
    k = min_c if min_c > 0 else min(1, max_c)   # smallest legal selection (skip when allowed)
    k = min(k, max_c, n)
    return list(range(k))
