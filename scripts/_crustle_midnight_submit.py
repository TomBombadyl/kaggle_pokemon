"""UTC day-flip: deep-confirm then submit ONLY Crustle (quality slot #1).

Policy lock (user Round 0):
  1. Submit only dist/candidates/crustle_MissingNo_v1.tar.gz
  2. Never submit Dragapult
  3. Deep gate (48 games) for stability log before ship
  4. Ship the known package even if short-gate noise is mid 鈥?illegal/crash aborts
...
ARCH = ROOT / "dist" / "candidates" / "crustle_MissingNo_v1.tar.gz"
LOG = Path(r"E:\PTCG_AI_Battle_Challenge\recordings\logs\post_midnight_crustle.log")
...
def reset_count() -> str:
    ...
    COUNT.write_text(
        json.dumps(
            {"date": day, "count": 0, "refs": [], "source": "crustle_midnight_v2"},
            indent=2,
        ),
        encoding="utf-8",
    )
...
def submit_arch() -> tuple[int, str]:
    proc = subprocess.run(
        [
            ...
            str(ARCH),
            "--message",
            "UTC day2 Crustle R7 quality#1 only (hist pin 1196.1 spine; no Dragapult)",
            "--local-gate",
            "65",
            "--strength-note",
...
""",
    ...
    try:
        state = f"""# PTCG AI Battle Challenge - 灏堟鐙€鎱嬭閷?**鏈€寰屾洿鏂?*锛歿local} (UTC {utc})

## 鐣跺墠鐙€鎱?- 鏈€寰屾洿鏂帮細{local}
- 鏈€楂樹及瑷堝挤搴︼細Crustle board ~793.7 (pre day2); hist pin **1196.1**
- 浠婃棩宸叉彁浜わ細day2 Arch-only attempt rc={rc}
- 涓昏鐡堕牳锛歝limb 793鈫?196; local deep gate wr={gate.get('wr')}
- 鏈€浣虫ā鍨嬶細`kaggle_pokemon/dist/candidates/crustle_MissingNo_v1.tar.gz`
- 鍌欒ɑ锛歊ound0 lock 鈥?no Dragapult; slot2 only if clear stable upgrade

## 瑷橀寗
- {km}
- {gate.get('log')}
- recordings/logs/post_midnight_crustle.log
"""
...
def main() -> int:
    ...
    # Stability note only 鈥?package is known champion spine; ship unless hard fail
