"""Wait until UTC daily Kaggle submit allowance resets, then submit Crustle.

Final strategy lock 2026-07-30 (chase #1 / pin 1196 鈫?1198+):
  - FIRST ship after UTC flip = Crustle ONLY (MissingNo #1 replica)
  - SECOND ship only if a clear Crustle upgrade exists (higher deep gate WR)
  - Never auto-ship Dragapult / Alakazam on day flip (demoted)
  - Max 1鈥? quality ships; board keeps latest 2
"""
...
# Day-flip: 1 guaranteed Crustle; 2nd only on clear upgrade evidence
MAX_QUALITY_SHIPS = 2
FIRST_SHIP_ONLY_CRUSTLE = True
# Require deep-gate WR before 2nd slot (must beat first ship local note)
SECOND_SHIP_MIN_WR_DELTA = 3.0

# Crustle-only queue (Dragapult/Alakazam demoted off day-flip path)
QUEUE = [
    (
        ROOT / "dist" / "candidates" / "crustle_MissingNo_v1.tar.gz",
        "Crustle MissingNo #1 day-flip #1 鈥?chase pin 1196 / top 1198+ (live~793.7)",
        200,
    ),
]
...
def reset_submit_count_for_new_day() -> None:
    ...
    day = utc_today()
    data = {"date": day, "count": 0, "refs": [], "source": "wait_and_submit_crustle_day_flip"}
...
def ordered_queue() -> list[tuple[Path, str, int]]:
    """Crustle-only day-flip queue (strategy lock 2026-07-30)."""
    items = [it for it in QUEUE if it[0].exists()]
    # Prefer newest crustle*.tar.gz if multiple variants appear later
    cand_dir = ROOT / "dist" / "candidates"
    extras: list[tuple[Path, str, int]] = []
    if cand_dir.exists():
        for p in sorted(cand_dir.glob("crustle*.tar.gz"), key=lambda x: x.stat().st_mtime, reverse=True):
            name_l = p.name.lower()
            # Never auto-pick broken backups / scratch renames
            if "broken" in name_l or name_l.startswith("zz_") or "_bak" in name_l:
                continue
            if not any(p.resolve() == it[0].resolve() for it in items):
                extras.append(
                    (
                        p,
                        f"Crustle variant {p.name} day-flip quality ship",
                        150,
                    )
                )
...
def try_submit(path: Path, message: str, local_gate: float = 60.0) -> tuple[bool, str]:
    ...
    if auto.exists():
        ...
        if proc.returncode == 0:
            ...
            if "[ok]" in low:
                return True, out[-500:]
            # exit 0 without skip 鈫?treat as ok
            return True, out[-500:]
        return False, out[-500:]
...
def _record_ship(path: Path, detail: str, day: str) -> None:
    ...
    row = {
        ...
        "strategy": "crustle_primary_day_flip",
    }
...
def _acquire_singleton_lock() -> object | None:
    """One day-flip waiter only — refuse second instance (avoids double-burn CAP)."""
    lock_path = ROOT / "dist" / "wait_submit_crustle.lock"
    lock_path.parent.mkdir(parents=True, exist_ok=True)
    try:
        fd = os.open(str(lock_path), os.O_CREAT | os.O_EXCL | os.O_WRONLY)
    except FileExistsError:
        # Stale lock: if no other wait process, reclaim
        try:
...
def main() -> int:
    ...
    if lock_fd is None:
        log("singleton_lock_held_exit", note="another wait_and_submit_crustle is already running")
        return 0
    log(
        "wait_and_submit start",
        py=str(PY),
        max_quality=MAX_QUALITY_SHIPS,
        strategy="crustle_only_1_to_2",
        target_pin=1196.1,
        target_top1=1198.4,
        pid=os.getpid(),
    )
    ...
    while submitted < MAX_QUALITY_SHIPS and attempts < max_attempts:
        ...
        for path, msg, _pri in ordered_queue():
            ...
            if not path.exists():
                ...
                continue
            # 2nd ship: only Crustle upgrade path (never other agents)
            if submitted >= 1:
                if "crustle" not in path.name.lower():
                ...
                if BEST_GATE_PATH.exists():
                    try:
                        bg = json.loads(BEST_GATE_PATH.read_text(encoding="utf-8"))
                        if str(bg.get("id", "")).startswith("crustle"):
                            wr = float(bg.get("wr") or 0)
                    except Exception:
                        pass
            ...
            if ok:
                ...
                if submitted == 1 and FIRST_SHIP_ONLY_CRUSTLE:
            ...
            if "DAILY_CAP" in detail or "allowance" in detail.lower():
                any_cap = True
                break
            # other error 鈫?try next package
        ...
        # one quality Archaludon shipped, no upgrade evidence 鈫?stop (don't spray)
